...
Five changes: trim-by-default on branch, dual auto-trim hooks (PreCompact + PostToolUse), batch branching with session status, and a read-only live JSONL session viewer. v3.0.0 makes the viewer interactive with PTY input routing.

**Implementation notes:**
- Session viewer uses JSONL file watcher (`fs.watch()`) instead of PTY. No native dependencies.
- PostToolUse hook stats the file first (~1ms) and only trims when over ~600KB threshold.

---

## Feature 0: Trim-by-Default on Branch

### What it does

`cmv branch` now trims by default. Use `--no-trim` for raw context. Breaking change to CLI behavior.

---

## Feature 1: Auto-Trim Hooks (PreCompact + PostToolUse)
...
## Feature 3: Read-Only Live Session Viewer

### What it does

When a branch is launched (via `Enter` or `m`), Claude runs in a PTY and its output streams into the right pane of the TUI. The user watches Claude work from the dashboard. They can navigate snapshots/sessions in the left/middle panes while Claude runs. The viewer is read-only in v2.0.0; the user types into Claude via a separate terminal (tmux pane, new tab, etc.).

### Why read-only first

Making the viewer interactive means routing stdin between Ink and the PTY based on pane focus. That's solvable but adds complexity around input conflicts, permission prompts, and edge cases. Read-only gives 80% of the value (see what Claude is doing across branches) with none of that risk.
...
Add as an optional dependency. If `node-pty` isn't available (install failed on a system without build tools), fall back to current behaviour (launch Claude externally).

```bash
npm install node-pty
```

#### 2. Session launcher with PTY
...
interface LiveSession {
  ...
  snapshotName: string;
  pty: IPty;
  buffer: string[];       // scrollback, last N lines
  startedAt: Date;
}
```

- `launchSession(sessionId, branchName, snapshotName, cwd)` — spawns `claude --resume {id}` in a PTY, returns `LiveSession`
- `listActive()` — returns all live sessions
- `stopSession(sessionId)` — sends SIGTERM, cleans up
- `onOutput(sessionId, callback)` — subscribes to PTY output stream
- `getBuffer(sessionId)` — returns current scrollback
- On TUI exit, clean up all PTYs

Buffer size: 500 lines per session. Older lines drop off.

#### 3. Terminal viewer component

**File: `src/tui/components/SessionViewer.tsx`**

- Receives a `LiveSession` (or null when nothing is live)
- Renders the scrollback buffer as `<Text>` elements inside a `<Box>`
- Strips ANSI cursor control sequences (CSI codes for cursor movement, line clearing, screen clearing)
- Preserves color/bold/underline via `chalk` or pass-through
- Auto-scrolls to bottom on new output
- Header shows: `{branchName} (from: {snapshotName}) ● Live`
- When no session is live, falls back to current DetailPane content

Use `strip-ansi` for cursor control and a lightweight ANSI-to-Ink parser for colors. Don't try to handle cursor positioning — just append lines.

#### 4. Dashboard integration

**File: `src/tui/Dashboard.tsx`**

- When user presses `Enter` on a branch:
  - If `node-pty` is available: spawn in PTY, show in right pane
  - If not: fall back to existing behaviour (exit TUI, launch externally)
- When user presses `m` for multi-branch:
  - Spawn all branches in PTYs
  - Right pane shows the first one
  - `Shift+Tab` cycles through live sessions (header updates to show which one)
- Right pane replaces `<DetailPane>` when any session is live
- When a PTY closes (Claude exits), mark session as Idle, if no other live sessions switch right pane back to DetailPane

#### 5. Layout change

Current: `ProjectPane | TreePane | DetailPane`

v2.0.0: `ProjectPane | TreePane | SessionViewer (or DetailPane when nothing is live)`

No structural layout change needed. The right column just conditionally renders either `<SessionViewer>` or `<DetailPane>`.

### Files to create/modify

| File | Action |
|------|--------|
| `src/core/process-manager.ts` | Create: PTY lifecycle management |
| `src/tui/components/SessionViewer.tsx` | Create: read-only terminal output viewer |
| `src/tui/Dashboard.tsx` | Modify: PTY launch on Enter/m, Shift+Tab cycling |
| `package.json` | Modify: add `node-pty` as optional dependency |

### Test plan

- Unit test: process-manager spawns a PTY, captures output, cleans up on stop
- Unit test: buffer rotation (oldest lines dropped after 500)
- Unit test: ANSI stripping removes cursor control, preserves colors
- Visual: launch TUI, press Enter on a branch, verify output streams into right pane
...
- Read-only live session viewer: Claude output streams into the TUI right pane
  via PTY. Shift+Tab cycles between live sessions. Falls back to DetailPane
...
9. **Process manager** (`process-manager.ts`) — PTY lifecycle
10. **Session viewer component** (`SessionViewer.tsx`) — ANSI rendering
11. **Dashboard PTY integration** — launch in PTY on Enter/m, Shift+Tab cycling
12. **Tests for all new features**
13. **README + CHANGELOG updates**

Steps 1-4 (auto-trim hook), 5-8 (batch branching), and 9-11 (session viewer) are independent and can be done in parallel.
...
## v3.0.0 Roadmap: Interactive Session Viewer

v2.0.0 ships the read-only viewer and the PTY infrastructure. v3.0.0 makes it interactive: you type into Claude directly from the TUI.

### What changes from v2.0.0

The right pane goes from read-only to read-write. Same layout, same PTY, same process manager. The new work is input routing.
...
- Only intercept Tab (switch to left/middle pane) and Shift+Tab (cycle sessions)

When left/middle pane is focused:
...
- **Ink stdin conflict**: Ink owns stdin. Forwarding raw keystrokes to a PTY while Ink is running may need `process.stdin.setRawMode(true)` and manual routing. Needs a proof-of-concept.
- **Memory**: each PTY + scrollback buffer adds overhead. 5 live sessions is probably fine, 20 might not be. Need to cap or implement buffer eviction.
- **Ink render performance**: with a PTY streaming output, Ink redraws on every state change. May need to throttle renders (batch PTY output into 100ms frames).
