...
```
┌─ Navigator ─────────────────┬─ Claude Session ──────────────────────┐
│ Projects                    │                                       │
│ > my-app                    │  auth-work (from: analysis)           │
│   other-project             │                                       │
│                             │  > read src/auth.ts                   │
│ Snapshots                   │                                       │
│ ▼ analysis (82 msgs)        │  The auth module uses JWT with        │
│   ├── auth-work    ● Live   │  refresh tokens stored in httpOnly    │
│   ├── api-work     ○ Idle   │  cookies...                           │
│   └── perf-fix     ○ Idle   │                                       │
│                             │                                       │
│ Sessions                    │  █                                    │
│   abc123...  42 msgs        │                                       │
└─────────────────────────────┴───────────────────────────────────────┘
  Tab: switch panes    Shift+Tab: cycle live sessions    m: multi-branch
```

- **Left pane focused**: CMV shortcuts work (b, t, s, d, m, etc.), j/k navigates
- **Right pane focused**: all input goes to Claude, CMV shortcuts disabled
- **Tab**: switch focus between left and right
- **Shift+Tab** (right pane focused): cycle through live Claude sessions, header shows snapshot + branch name so you know what you're looking at
- Tree view splits into Snapshots and Sessions sections, branches show Live/Idle status
...
v3.0.0 TUI: two columns (NavigatorPane, TerminalPane)

NavigatorPane combines the current ProjectPane and TreePane into one scrollable view:
- Projects section at the top
- Snapshots section (with branches inline, showing Live/Idle)
- Sessions section at the bottom

DetailPane content (context analysis, metadata) moves into the NavigatorPane as a collapsible section, or shown on hover/select.
