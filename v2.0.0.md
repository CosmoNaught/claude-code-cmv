...
Two features: auto-trim via Claude Code's hook system, and parallel session management in the TUI.
...
## Feature 2: Parallel Session Management

### What it does

Branch multiple times from the same snapshot and see all running sessions in the TUI. Not orchestration, not task decomposition. Just CMV's existing branching, but with visibility into multiple active sessions at once.

### Current limitations

The TUI currently:
- Shows sessions as flat leaf nodes with basic metadata
- Can only launch one Claude instance (exits TUI, spawns Claude, TUI is gone)
- Has no concept of "active" vs "idle" sessions beyond a 2-minute mtime heuristic
- Cannot monitor multiple sessions simultaneously
...
#### 4. Quick-switch between sessions

When viewing a branch in the tree and pressing `Enter`, CMV currently exits the TUI and launches Claude with `--resume`. For parallel session management, add `Shift+Enter` or `r` to resume a session without leaving the TUI first (spawns Claude in background/new terminal).

Actually, this is platform-dependent and complex. Simpler approach:

- `Enter` on a branch: exits TUI, resumes that session (existing behavior)
- The user switches between sessions by re-launching `cmv` and picking a different branch

This is less flashy but works everywhere. The value is in the **visibility** (seeing all branches + their status at a glance), not in running them simultaneously from one terminal.

#### 5. Aggregate context view in DetailPane
...
- Multi-branch from TUI: press `m` on a snapshot to create multiple branches at once.
...
Steps 1-4 (auto-trim hook) and 5-9 (parallel sessions) are independent and can be done in parallel by two instances if needed.
